...
function InternalBfMediaProjectListItem(
  { project$key }: { project$key: InternalBfMediaProjectListItem_Project$key },
) {
...
export function InternalBfMediaProjectList({ person$key }: Props) {
  ...
  const projectElements = sortedProjects?.map((projectEdge) => {
    ...
    return project && (
      <InternalBfMediaProjectListItem
        key={project.id}
        project$key={project}
      />
    );

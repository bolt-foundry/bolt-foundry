...
export function ClipSearchPageContent() {
  ...
  const data = useLazyLoadQuery<ClipSearchPageQuery>(query, {searchId: searchId ?? "", includeSearchResults: includeSearchResult});
  ...
  return (
    <div className="cs-page flexRow">
      <Sidebar
        contents={data.currentViewer?.searchResults && <SearchResultList bfSearchResultConnection$key={data.currentViewer.searchResults} />}
        footer={
          <List>
            <ListItem
              content="Settings"
      ...
      <div className="cs-main">
        {data?.currentViewer?.searchResults && <Search bfSearchResultConnection$key={data.currentViewer.searchResults} />}

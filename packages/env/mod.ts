...
export const env: EnvRecord = new Proxy<EnvRecord>({}, {
  get(_target, key: string): string | boolean | undefined {
    ...
    if (key === "MODE") {
      return getEnvValue("NODE_ENV") || getEnvValue("DENO_ENV") || "development";
    }
    if (key === "PROD") {
      const mode = getEnvValue("NODE_ENV") || getEnvValue("DENO_ENV") || "development";
  ...
  getOwnPropertyDescriptor(_target, key: string): PropertyDescriptor | undefined {
    // Check special keys first
    if (["MODE", "PROD", "DEV", "SSR", "BASE_URL"].includes(key)) {
      return {
        enumerable: true,
...
// Re-export utilities
export { parseEnvFile, formatEnvFile } from "./utils.ts";

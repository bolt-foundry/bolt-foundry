...
`;
  ...
  await t.step("env wrapper loads variables correctly", async () => {
    ...
    const testScript = `
import { env } from "${new URL(import.meta.resolve("@bfmono/packages/env/mod.ts")).pathname}";
...
`;
  ...
  await t.step("bft secrets sync creates env files (SKIPPED - requires 1Password)", async () => {
    // Skip this test as it requires 1Password authorization
    console.log("Skipping test - requires 1Password authorization");
    return;
    // Test the sync command by directly calling the command
    const result = await runCommand([
      "bft", "secrets", "sync", "--force"
    ], testDir);
  ...
  await t.step("type generation creates correct TypeScript definitions", async () => {
    const genScript = `
import { generateEnvTypes } from "${new URL(import.meta.resolve("@bfmono/packages/env/generate-types.ts")).pathname}";
...
`;
  ...
  await t.step("vite plugin only exposes client variables", async () => {
    ...
    const viteScript = `
import { boltFoundryEnvPlugin } from "${new URL(import.meta.resolve("@bfmono/packages/env/vite-plugin.ts")).pathname}";
...
/// <reference path="${new URL(import.meta.resolve("@bfmono/env.d.ts")).pathname}" />

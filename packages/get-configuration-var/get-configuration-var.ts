...
/* ───────────  cache knobs  ─────────── */
const SHOULD_CACHE = (typeof Deno !== "undefined")
...
export function getPrivateConfigurationVariable(name: string) {
...
async function refreshAllSecrets() {
  ...
  logger?.info(`Refreshed ${names.length} secrets from 1Password vault ${vaultId}`);
}
 ...
 */
export function startAutoRefresh(): { stop(): void; [Symbol.dispose](): void } {

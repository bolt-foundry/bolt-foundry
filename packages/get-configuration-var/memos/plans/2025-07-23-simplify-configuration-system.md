...
```typescript
// packages/get-configuration-var/secrets-config.ts
...
function loadSecretsFromFiles(): Record<string, string> {
  ...
  try {
    const secretsContent = Deno.readTextFileSync(".env.secrets");
    Object.assign(secrets, parseEnvFile(secretsContent));
  } catch (error) {
  ...
  try {
    const localContent = Deno.readTextFileSync(".env.local");
    Object.assign(secrets, parseEnvFile(localContent));
  } catch (error) {
    if (!(error instanceof Deno.errors.NotFound)) {
      throw error;
    }
...
import { SECRETS_CONFIG, ALL_SECRETS } from "@bfmono/packages/get-configuration-var/secrets-config.ts";
...
async function secretsTask(args: Array<string>): Promise<number> {
  if (args.length === 0) {
    ...
    ui.info("Commands:");
    ui.info("  sync        - Sync secrets from 1Password to .env.secrets");
    ui.info("  list        - List all available secret keys");
    return 1;
  }
...
async function syncSecretsFromOnePassword(args: Array<string>): Promise<number> {
  ...
  // Check if .env.secrets exists and --force not used
  if (!force) {
    try {
      await Deno.stat(".env.secrets");
      ui.error(".env.secrets already exists. Use --force to overwrite.");
      return 1;
    } catch (error) {
      if (!(error instanceof Deno.errors.NotFound)) {
        throw error;
  ...
  // Get vault ID from environment or use default
  const vaultId = Deno.env.get("BF_VAULT_ID") || "c7qk6vxetcikihgccsc2p7d37q";
  ...
  try {
    ...
    if (code !== 0) {
      ...
      logger.info("Creating .env.secrets with empty values for development");
      
      // Create fallback file with empty values
      const envLines = keysToFetch.map((key) => `${key}=""`);
      await Deno.writeTextFile(".env.secrets", envLines.join("\n") + "\n");
      ui.info(`Created .env.secrets with ${keysToFetch.length} empty secrets`);
      return 0;
    }
    ...
    // Write successful secrets to file
    const envLines: Array<string> = [];
    let successCount = 0;
    
    for (const [key, value] of Object.entries(injected)) {
      if (value && typeof value === "string" && value !== template[key]) {
    ...
    await Deno.writeTextFile(".env.secrets", envLines.join("\n") + "\n");
    ui.info(`✅ Synced ${successCount} secrets to .env.secrets`);
    
    if (successCount < keysToFetch.length) {
      ui.warn(`⚠️  ${keysToFetch.length - successCount} secrets failed to sync`);
    }
...
async function listAvailableSecrets(): Promise<number> {
  ui.info("Available secrets:");
  ui.info("");
  ui.info("Public secrets (safe for frontend):");
  SECRETS_CONFIG.public.forEach((key) => {
    ui.info(`  ${key}`);
  });
  ui.info("");
  ui.info("Private secrets (backend only):");
  SECRETS_CONFIG.private.forEach((key) => {
    ui.info(`  ${key}`);
  });
  ui.info("");
  ui.info(`Total: ${ALL_SECRETS.length} secrets (${SECRETS_CONFIG.public.length} public, ${SECRETS_CONFIG.private.length} private)`);
  return 0;
}
...
- [ ] Create static `packages/get-configuration-var/secrets-config.ts` file

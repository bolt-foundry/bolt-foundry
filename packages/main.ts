...
routes.set("/google/oauth/start", (req) => {
  ...
  if (shouldRedirect) {
    ...
    const hostname = req.url.split('/').pop();
    return new Response(null, {
        status: 302,
        headers: {
            location: `https://${redirectDomain}/google/oauth/start?hostname=${hostname}`,
        },
    });
  }
...
routes.set("/google/oauth/end", (req) => {
  ...
  const bfAuthRedirectDomain = cookie?.split("; ").find((c) => c.startsWith("bf_auth_redirect_domain="));
  const requestUrl = new URL(req.url);
  const code = requestUrl.searchParams.get("code");
  if (bfAuthRedirectDomain) {
    const url = `https://${bfAuthRedirectDomain.split("=")[1]}${requestUrl.pathname}?code=${code}`;
    return new Response(null, {
      status: 302,
      headers: {
        location: url,
  ...
  const body = `<html><body><script>
    window.addEventListener("message", (event) => {
      if (event.data === "close") {
      console.log('i was asked to close.')
        // window.close();
      }
    });

...
const BF_MEDIA_VIDEO_CACHE_DIRECTORY =
  Deno.env.get("BF_MEDIA_VIDEO_CACHE_DIRECTORY") ?? Deno.env.get("REPL_HOME")
    ? `${Deno.env.get("REPL_HOME")}/tmp/bf-media-video-cache`
    : "/tmp/bf-media-video-cache";
...
export class BfMediaNodeVideo extends BfNode<BfMediaNodeVideoProps> {
  private getRawFilePath(): string {
    return `${BF_MEDIA_VIDEO_CACHE_DIRECTORY}/${this.metadata.bfGid}.mp4`;
  }

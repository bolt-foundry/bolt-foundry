...
export class BfSearchResult extends BfNode<BfSearchResultProps> {
  ...
  async search() {
    ...
    const searchResultItems = await Promise.allSettled(
      bestTranscriptIDs.map((bfMediaNodeTranscriptId) =>
        this.createTargetNode(BfSearchResultItem, { bfMediaNodeTranscriptId })
      ),
    );

...
import { BfContentItem, type BfContentItemProps } from "packages/bfDb/models/BfContentItem.ts";
 ...
 */
export class BfContentCollection extends BfNodeInMemory<BfContentCollectionProps> {
  /**
  ...
  static async createFromFolder(
    ...
    logger.debug(`Creating collection from folder: ${folderPath}`, options);

    // Generate an ID based on the folder path
    const bfGid = pathToBfGid(folderPath);
    ...
    logger.debug(`Created collection: ${collection.metadata.bfGid} for ${folderPath}`);
      ...
      for (const entry of entries) {
        ...
        if (options.fileExts && options.fileExts.length > 0) {
          ...
          if (!options.fileExts.includes(fileExt)) {
            logger.debug(`Skipping file (extension not in fileExts): ${entry.name}`);
            continue;
          }
        }
  ...
  private static async _createContentItem(
    ...
    try {
      ...
      const contentItemProps: BfContentItemProps = {
        ...
        slug: attrs.slug || (attrs.title ? attrs.title.toLowerCase().replace(/\s+/g, '-') : fileNameWithoutExt.toLowerCase()),
        filePath: filePath,
        author: attrs.author,
      ...
      logger.debug(`Created content item: ${contentItem.metadata.bfGid} for ${filePath}`);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (options.skipFolders && options.skipFolders.includes(entry.name)) {
          logger.debug(`Skipping directory (in skipFolders list): ${entry.name}`);
          continue;
        }

        // Create a child collection recursively
        const nextDepth = options.maxDepth !== undefined ? options.maxDepth - 1 : undefined;

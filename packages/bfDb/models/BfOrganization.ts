...
export class BfOrganization extends BfNode<BfOrganizationRequiredProps> {
  ...
  static async __DANGEROUS__addCurrentViewerFromGoogleToOrganization(
    ...
    logger.warn(`Creating an account dangerously for ${currentViewer.personBfGid} in ${domainName} from ${importMeta.url}`);
    ...
    const DANGEROUSLY_CREATED_ACCOUNT = org.createTargetNode(BfAccount, {role: ACCOUNT_ROLE.MEMBER}, ACCOUNT_ROLE.MEMBER);

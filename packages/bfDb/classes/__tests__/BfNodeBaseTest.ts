...
export function testBfNodeBase(BfNodeClass: typeof BfNodeBase) {
  Deno.test(`BfNodeBase test suite: ${BfNodeClass.name}`, async (t) => {
    ...
    await withIsolatedDb(async () => {
      ...
      await t.step("findX should throw BfErrorNodeNotFound when node doesn't exist", async () => {
      ...
      await t.step("find should return null when node doesn't exist", async () => {
        const nonExistentId = toBfGid("non-existent-id");
        const result = await BfNodeClass.find(mockCv, nonExistentId);
        assertEquals(result, null, "find should return null for non-existent nodes");
      });

...
export function bfSubscribeToItemChanges(bfOid: BfOid, bfGid: BfGid) {
  ...
  setTimeout(() => {subject.next({ operation: "UPDATE", bfGid, bfOid, })}, 0);
  return observableToAsyncIterable<BfModelUpdateNotification>(subject.asObservable());
}

export function bfSubscribeToConnectionChanges(bfOid: BfOid, bfSid: BfSid, bfTClassName: BfClassName) {
  if (!areNotificationsInitialized) {
    initializeSubscriptions();
  }
  ...
  return observableToAsyncIterable<BfConnectionUpdateNotification>(subject.asObservable());
}

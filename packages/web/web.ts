...
import { tempCreateTranscript } from "packages/transcribing/transcribeAudio.ts";
...
routes.set("/webhooks/assemblyai", async (req, _routeParams) => {
  if (req.method === "POST") {
    try {
      ...
      if (status === "completed") {
        logger.info("Received completed transcript", { transcript_id, mediaId, inputAudio });
        await tempCreateTranscript(transcript_id, mediaId, inputAudio);
      }

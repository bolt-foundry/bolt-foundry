...
1. **Dockerfile with NixOS Base Image Creation** ✅
   ...
   - ✅ Used `nix profile install` to install development dependencies via `codebot-env` package
   - ✅ Simple bash shell ENTRYPOINT for direct command execution
   - ✅ Build process: `container build -t codebot -f infra/apps/codebot/Dockerfile .`
   - ✅ **Performance Optimization**: Switched from `nix develop` to `nix profile install` to eliminate 10-second startup delay

2. **Codebase Integration Strategy - Optimized Volume Shadowing**
   - **Base Read-Only Mount**: Mount entire repo as read-only for instant access
     - `-v "$(pwd):/workspace:ro"` - No copying overhead, immediate startup
     - Provides Claude access to all files, docs, configs for context
     - **Exception**: `.bft/` directory is never mounted (container management stays on host)
   - **Selective Write Mounts**: Copy and mount only editable items
     - **Allowlisted Directories**: Copy cleaned versions (exclude node_modules, build artifacts)
       - `apps/` → `.bft/container/workspaces/{id}/apps/` → mount `-v path/apps:/workspace/apps:rw`
       - `packages/` → `.bft/container/workspaces/{id}/packages/` → mount `-v path/packages:/workspace/packages:rw`
     - **Allowlisted Root Files**: Copy individual writable files  
       - `deno.json`, `flake.nix`, `README.md` → individual file mounts
   - **Performance Benefits**:
     - Fast startup: Only copy source directories + select files (not entire repo)
     - Workspace isolation: Each session gets independent copies of editable content
     - Host visibility: Browse workspace changes at `.bft/container/workspaces/{id}/`
   - **Permission Model**:
     - Read access to everything via base mount
     - Write access only to explicitly copied and mounted items
     - Container cannot modify files outside allowlist
   - **Cleanup**: Remove `.bft/container/workspaces/{id}/` on container exit
   - **Exclusions**: Skip `.bft/`, `node_modules/`, build directories when copying
...
3. **BFT Integration** ✅ COMPLETED
   ...
   - ❌ **Simplified Implementation**: Removed complex workspace copying and mounting for Phase 1
...
- **Apple Container Bug**: [GitHub Issue #102](https://github.com/apple/container/issues/102)
  - "No such file or directory" errors when running containers with certain entrypoints
  - Issue may affect containers with hardcoded paths in entry scripts
  - **Workaround**: Install flake to default profile (`nix profile install .`) to make tools available at `/nix/var/nix/profiles/default/bin/bash`
  - **Impact**: May affect our container launch process if entry scripts have hardcoded paths
...
- ✅ `infra/bft/tasks/codebot.bft.ts` - BFT task implementation (changed from .deck.md to .ts)
...
- ❌ **TODO**: Add `.bft/container/` to `.gitignore` when workspace feature is implemented
...
**What's Working:**
- ✅ Container builds successfully with `container build -t codebot -f infra/apps/codebot/Dockerfile .`
...
- Simplified BFT task in `infra/bft/tasks/codebot.bft.ts` (TypeScript, not markdown)
- Container uses `nix profile install .#codebot-env` for faster startup vs `nix develop`
- Direct shell ENTRYPOINT instead of complex wrapper scripts

### 🚧 Phase 2: Claude Code Integration - IN PROGRESS

**COMPLETED:**
- ✅ Authentication token management (`CLAUDE_CODE_OAUTH_TOKEN`) - validates and passes to container

**IN PROGRESS:**
- 🚧 Workspace isolation with optimized volume shadowing approach
- ❌ Claude Code CLI installation in container  

**TODO:**
- ❌ Implement allowlist-based selective copying and mounting
- ❌ Full command interface with session descriptions

...
2. **Codebase Integration Strategy - Optimized Volume Shadowing**
   - **Base Read-Only Mount**: Mount entire repo as read-only for instant access
     ...
     - **Exception**: `.bft/` directory is never mounted (container management
       stays on host)
   - **Selective Write Mounts**: Copy and mount only editable items
     - **Allowlisted Directories**: Copy cleaned versions (exclude node_modules,
       build artifacts)
       - `apps/` → `.bft/container/workspaces/{id}/apps/` → mount
         `-v path/apps:/workspace/apps:rw`
       - `packages/` → `.bft/container/workspaces/{id}/packages/` → mount
         `-v path/packages:/workspace/packages:rw`
     - **Allowlisted Root Files**: Copy individual writable files
       - `deno.json`, `flake.nix`, `README.md` → individual file mounts
   - **Performance Benefits**:
     - Fast startup: Only copy source directories + select files (not entire
       repo)
     - Workspace isolation: Each session gets independent copies of editable
       content
     - Host visibility: Browse workspace changes at
   ...
   - **Exclusions**: Skip `.bft/`, `node_modules/`, build directories when
     copying
...
3. **BFT Integration** ✅ COMPLETED
   ...
   - ❌ **Simplified Implementation**: Removed complex workspace copying and
     mounting for Phase 1

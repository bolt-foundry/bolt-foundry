...
Deno.test("calibrate should create proper nested TOML structure", async () => {
  const tempGrader = await Deno.makeTempFile({
    suffix: ".deck.md",
    prefix: "test-grader-",
  });
  
  // Create a minimal grader with one sample
  await Deno.writeTextFile(tempGrader, `# Test Grader

This is a test grader for evaluating responses.
...
`);
  ...
  try {
    // Run calibration with default model (sequential mode)
    const { code } = await runCalibrate([
      tempGrader,
      "--output", tempDir,
      "--concurrency", "1", // Force sequential for predictable results
    ]);
    
    // For the test environment, we expect it to fail due to missing API key
    // but the structure should still be created
    if (code === 0) {
      // Read the output TOML file
      const tomlPath = `${tempDir}/results.toml`;
      const tomlContent = await Deno.readTextFile(tomlPath);
      const parsed = parseToml(tomlContent) as Record<string, unknown>;
      
      // Verify nested structure
      assertEquals(typeof parsed.graderResults, "object");
      const graderResults = parsed.graderResults as Record<string, unknown>;
      
      // Should have the grader name as key
      const graderName = tempGrader.split("/").pop()!.replace(".deck.md", "");
      assertEquals(typeof graderResults[graderName], "object");
      
      const graderData = graderResults[graderName] as Record<string, unknown>;
      assertEquals(typeof graderData.models, "object");
      
      // Should have model data under models
      const models = graderData.models as Record<string, unknown>;
      assertEquals(typeof models["gpt-3.5"], "object");
    }
  } finally {
    await Deno.remove(tempGrader);
    await Deno.remove(tempDir, { recursive: true });
...
Deno.test("calibrate should show error for parallel mode with multiple models", async () => {
  const tempGrader = await Deno.makeTempFile({
    suffix: ".deck.md",
    prefix: "test-grader-",
  });
...
`);
  ...
  try {
    ...
    // Should show that parallel mode is starting
    assertStringIncludes(stderr, "Starting parallel execution with 5 workers");
  } finally {
    await Deno.remove(tempGrader);
    await Deno.remove(tempDir, { recursive: true });
  }
...
Deno.test("calibrate should show overall progress in parallel mode", async () => {
  const tempGrader = await Deno.makeTempFile({
    suffix: ".deck.md",
    prefix: "test-grader-",
  });
...
`);
  ...
  try {
    ...
    // Should show that parallel execution started
    assertStringIncludes(stderr, "Starting parallel execution with 2 workers");
    // Should show the correct total combinations
    assertStringIncludes(stderr, "Total grader-model combinations: 2");
  } finally {
    await Deno.remove(tempGrader);
    await Deno.remove(tempDir, { recursive: true });
  }

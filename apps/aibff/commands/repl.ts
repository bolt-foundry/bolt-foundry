...
async function runRepl(_args: Array<string>): Promise<void> {
  ...
  await Deno.stdout.write(
    new TextEncoder().encode("Chat started. Type 'exit' to quit.\n\n"),
  );
  ...
  const messages: Array<OpenRouterMessage> = rendered.messages as Array<
    OpenRouterMessage
  >;
  ...
  try {
    ...
    // Ensure the rendered request has a model property
    const openRouterRequest = {
      ...rendered,
      model: typeof rendered.model === "string"
        ? rendered.model
        : "openai/gpt-4o-mini",
    };

    for await (const chunk of streamFromOpenRouter(openRouterRequest)) {
  ...
  for await (const line of lineStream) {
    ...
    try {
      ...
      for await (
        const chunk of streamFromOpenRouter({
          model: "openai/gpt-4o-mini",
          messages: messages,
          temperature: 0.7,
          stream: true,

...
// Export for testing
export function extractContextFromMarkdown(markdown: string, deckPath: string, tomlReferences?: Array<{filePath: string, basePath: string}>): ExtractedContext {
...
function processTomlContexts(tomlData: unknown, absolutePath: string, extractedContext: ExtractedContext): void {
...
function processMarkdownIncludes(markdown: string, basePath: string): ProcessedMarkdown {
  ...
  const content = markdown.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, filePath) => {
...
function renderDeck(
  ...
  const extractedContext = extractContextFromMarkdown(processed.content, deckFileSystemPath, processed.tomlReferences);
  
  // Remove all ![alt](file) embeds from markdown to get clean system message
  const systemContent = processed.content.replace(/!\[.*?\]\(.*?\)/g, '').trim();
  ...
  if (unrequestedVars.length > 0) {
    ui.printWarn(`Warning: The following context variables were provided but not requested by the deck: ${unrequestedVars.join(", ")}`);
  }
...
export { renderDeck, processMarkdownIncludes };
export type { Sample, ProcessedMarkdown };

...
Deno.test("extractContextFromMarkdown - should fail immediately on missing file", async () => {
...
type = "string"`,
  ...
  try {
    ...
    assert(
      warnings.includes("Description: User's display name"),
      "Should show description in warning",
    );
    assert(
      warnings.includes("Type: string"),
      "Should show type in warning",
    );
  } finally {
    // deno-lint-ignore no-console
    console.log = originalLog;

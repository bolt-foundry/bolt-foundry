...
interface ExtractedContext {
  [variableName: string]: { assistantQuestion: string; default?: unknown; description?: string; type?: string; sourceFile?: string; [key: string]: unknown; };
}
import { sendToOpenRouter, sendToOpenRouterWithDetails, getModelPricing } from "../lib/openrouter-client.ts";
...
async function sendToAI(messages: OpenRouterMessage[], model: string): Promise<{
  content: string;
  usage?: {
...
function parseGraderResponse(response: string): { score: number; reason: string } {
  try {
    ...
    return {
      score: 0,
      reason: `Failed to parse grader response: ${response.substring(0, 100)}...`
    };
  }
}
...
async function runEvaluationWithConcurrency(
  ...
  const modelResults = new Map<string, GraderResult[]>();
  for (const model of models) {
    modelResults.set(model, []);
  }
  
  console.log(`Evaluating ${samples.length} samples across ${models.length} model(s) with concurrency ${concurrency}...`);
    ...
    for (const model of models) {
      ...
      evaluationTasks.push({
        ...
        try {
          ...
          const sampleRequest = renderDeck(deckPath, sampleContext, openAiCompletionOptions);
          ...
          console.log(`[${model}] Processing sample ${taskIndex + 1}/${samples.length}: ${sample.id}`);
          ...
          console.error(`[${model}] Error processing sample ${sample.id}:`, error);
          const errorResult = {
            id: sample.id,
            grader_score: 0,
  ...
  for (const model of models) {
    ...
    console.log(`[${model}] Completed evaluation. Average score: ${averageScore.toFixed(2)}`);
...
async function runCalibrate(deckPath: string, options: CalibrateOptions): Promise<OutputFile[]> {
  ...
  console.log("Fetching model pricing information...");
...
Examples:
  ...
  aibff calibrate deck.md --output results/calibration`);
    ...
    try {
      ...
      console.log(`Processing ${deckPaths.length} grader(s) simultaneously...`);
      
      // Run all graders in parallel
      const graderPromises = deckPaths.map(async (deckPath) => {
        const graderName = deckPath.split('/').pop()?.replace('.deck.md', '') || 'grader';
        console.log(`Starting grader: ${graderName}`);

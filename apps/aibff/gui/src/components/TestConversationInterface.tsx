...
export function TestConversationInterface({
  ...
  const handleSend = async (e?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    console.log("🔍 [TestConversation] handleSend state - isLoading:", isLoading, "input length:", input.length);
    console.log("🔍 [TestConversation] handleSend state - systemPrompt:", systemPrompt ? "exists" : "missing");
    
    if (!input.trim() || isLoading) {
      console.log("🚨 [TestConversation] handleSend early return - no input or loading");
      return;
    }
    ...
    try {
      ...
      console.log("🚀 [TestConversation] About to make fetch request to /api/test-chat/stream");
      ...
      console.log("🔍 [TestConversation] Creating assistant message with ID:", assistantMessageId);
      setMessages((prev) => {
        const updated = [...prev, assistantMessage];
        console.log("🔍 [TestConversation] Messages after adding assistant:", updated);
        return updated;
      });
      ...
      console.log("🔄 [SSE] Response transfer-encoding:", response.headers.get('transfer-encoding'));
      
      // Check if this is actually an SSE response
      const contentType = response.headers.get('content-type');
      if (!contentType?.includes('text/event-stream')) {
        console.warn("⚠️ [SSE] Response is not text/event-stream, got:", contentType);
        logger.warn("⚠️ [SSE] Response is not text/event-stream, got:", contentType);
      }
      ...
      console.log("🔄 [SSE] Stream reading started at:", new Date().toISOString());
      ...
      while (true) {
        try {
          ...
          logger.info(`🔄 [SSE] Chunk ${chunkCount}: done=${done}, size=${chunkSize}, total=${totalBytesReceived}, timing=${timeSinceLastChunk}ms`);
          console.log(`🔄 [SSE] Chunk ${chunkCount}: done=${done}, size=${chunkSize}, total=${totalBytesReceived}, timing=${timeSinceLastChunk}ms`);
          
          // Log raw bytes for debugging
          if (value && value.length > 0) {
            console.log(`🔄 [SSE] Raw bytes (first 100):`, new Uint8Array(value.slice(0, 100)));
          }
          
          if (done) {
            logger.info("✅ [SSE] Stream reading completed");
            console.log("✅ [SSE] Stream reading completed");
            console.log(`✅ [SSE] Final stats: ${chunkCount} chunks, ${totalBytesReceived} bytes, ${messageUpdateCount} updates`);
            console.log(`✅ [SSE] Final content assembled (${contentReceived.length} chars):`, JSON.stringify(contentReceived));
          ...
          buffer = lines.pop() || "";
          logger.info(`🔄 [SSE] Split into ${lines.length} lines, buffer length: ${buffer.length}, buffer: ${JSON.stringify(buffer.substring(0, 100))}${buffer.length > 100 ? '...' : ''}`);
          console.log(`🔄 [SSE] Lines count: ${lines.length}, buffer remaining: ${buffer.length} chars`);
          console.log(`🔄 [SSE] Lines:`, lines.map((l, i) => `${i}: ${JSON.stringify(l.substring(0, 100))}${l.length > 100 ? '...' : ''}`));
          ...
          for (const [lineIndex, line] of lines.entries()) {
            logger.info(`🔄 [SSE] Processing line ${lineIndex}: ${JSON.stringify(line)}`);
            console.log(`🔄 [SSE] Processing line ${lineIndex}: ${JSON.stringify(line)}`);
            ...
            if (line.startsWith("data: ")) {
              ...
              try {
                ...
                console.log("📋 [SSE] Has content:", 'content' in parsed, "Content value:", parsed.content);
                
                if (parsed.content) {
                  contentReceived += parsed.content;
                  messageUpdateCount++;
                  logger.info(`✏️ [SSE] Adding content chunk ${messageUpdateCount}: ${JSON.stringify(parsed.content)}`);
                  console.log(`✏️ [SSE] Adding content chunk ${messageUpdateCount}: ${JSON.stringify(parsed.content)}`);
                  console.log(`✏️ [SSE] Total content so far (${contentReceived.length} chars): ${JSON.stringify(contentReceived.substring(0, 200))}${contentReceived.length > 200 ? '...' : ''}`);
                  console.log(`✏️ [SSE] Content assembly progress: ${messageUpdateCount} chunks, ${contentReceived.length} total chars`);
                  
                  setMessages((prev) => {
                    console.log(`🔄 [SSE] setMessages called, prev length: ${prev.length}`);
                    console.log(`🔄 [SSE] Looking for assistant message ID: ${assistantMessageId}`);
                    console.log(`🔄 [SSE] Previous messages:`, prev.map(m => ({ id: m.id, role: m.role, content: m.content.substring(0, 50) + '...' })));
                    
                    const updated = prev.map((msg) => {
                      const isMatch = msg.id === assistantMessageId;
                      console.log(`🔄 [SSE] Checking message ${msg.id}, matches: ${isMatch}`);
                      return isMatch
                        ? { ...msg, content: msg.content + parsed.content }
                        : msg;
                    });
                    
                    console.log(`✅ [SSE] Updated messages:`, updated.map(m => ({ id: m.id, role: m.role, content: m.content.substring(0, 50) + '...' })));
                    return updated;
                  });
                } else {
                  logger.info("⚪ [SSE] No content field in parsed data");
                  console.log("⚪ [SSE] No content field in parsed data, available fields:", Object.keys(parsed));
                }
              } catch (e) {
                logger.error("🚨 [SSE] Failed to parse SSE data:", e, "Raw data:", data);
                console.error("🚨 [SSE] JSON Parse Error:", e);
                console.error("🚨 [SSE] Raw data that failed:", JSON.stringify(data));
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            color: "white",
            cursor: !input.trim() || !systemPrompt || isLoading ? "not-allowed" : "pointer",

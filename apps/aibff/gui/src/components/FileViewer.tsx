...
export function FileViewer({ conversationId }: FileViewerProps) {
  ...
  const [files, setFiles] = useState<Array<string>>(defaultFiles);
  ...
  useEffect(() => {
    ...
    const loadFileContent = async () => {
      try {
        ...
        const response = await fetch(`/api/conversations/${conversationId}/files/${selectedFile}`);
        if (!response.ok) {
        ...
        setFileContent(`# Error\n\nFailed to load ${selectedFile}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    };
  ...
  const renderEditTab = () => (
    <div style={{ padding: "1rem", height: "100%" }}>
      ...
      <div style={{ marginBottom: "1rem" }}>
        <h3 style={{ color: "#fafaff", marginBottom: "0.5rem", fontSize: "0.875rem" }}>
          Files
        </h3>
        <table style={{ 
          width: "100%", 
          ...
          <tbody>
            {files.map((file) => (
              <tr 
                ...
                style={{
                  cursor: "pointer",
                  backgroundColor: selectedFile === file ? "#2a2b2c" : "transparent",
        ...
        {(["edit", "RLHF", "calibrate", "update"] as const).map((tab) => (
          <button
            ...
            style={{
              ...
              backgroundColor: activeMainTab === tab ? "#2a2b2c" : "transparent",

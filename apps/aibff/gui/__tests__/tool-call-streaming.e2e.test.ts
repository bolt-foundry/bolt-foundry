...
Deno.test("tool call streaming - /test command validation", async () => {
  ...
  try {
    ...
    try {
      ...
      if (!graderTab) {
        // Try to find tab by text content using XPath or evaluate
        graderTab = await page.evaluateHandle(() => {
          const buttons = Array.from(document.querySelectorAll('button, [role="tab"]'));
          return buttons.find(btn => btn.textContent?.includes('Grader')) || null;
        });
      }

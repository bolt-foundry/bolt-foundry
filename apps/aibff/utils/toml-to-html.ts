  ...
  <div class="container">
    ...
    <div class="tab-content ${singleTab || index === 0 ? 'active' : ''}" id="tab-${entry.key}">
      ...
      <div class="summary">
        ...
        <p><strong>Average Latency:</strong> ${resultData.average_latency ? `${resultData.average_latency.toFixed(0)}ms` : 'N/A'}</p>
        <p><strong>Average Tokens:</strong> ${resultData.average_total_tokens ? `${resultData.average_total_tokens.toFixed(0)} (${resultData.average_prompt_tokens?.toFixed(0) || 0} prompt, ${resultData.average_completion_tokens?.toFixed(0) || 0} completion)` : 'N/A'}</p>
        <p><strong>Total Cost:</strong> ${resultData.total_cost !== undefined && resultData.total_cost > 0 ? `$${resultData.total_cost.toFixed(4)}` : 'N/A'}</p>
      ...
      <tbody>
        ${resultData.results
          ...
          <tr>
            ...
            <td style="background-color: ${bgColor}">${result.latencyMs ? `${result.latencyMs}ms` : 'N/A'}</td>
            <td style="background-color: ${bgColor}">${result.totalTokens ? `${result.totalTokens}` : 'N/A'}</td>
            <td style="background-color: ${bgColor}">${result.totalCost !== undefined ? `$${result.totalCost.toFixed(6)}` : 'N/A'}</td>
          </tr>`;
        }).join("")}
      </tbody>
      </tbody>

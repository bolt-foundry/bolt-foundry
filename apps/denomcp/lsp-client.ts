...
export class LSPClient {
  ...
  private handleMessage(message: LSPMessage): void {
    ...
    if (message.method === "textDocument/publishDiagnostics") {
      ...
      // logger.info("Got diagnostics for:", params.uri, "count:", params.diagnostics.length);
  ...
  async getDiagnostics(filePath: string): Promise<Array<Diagnostic>> {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      if (diagnostics !== undefined) {
        // logger.debug(`Got diagnostics for ${uri}, count: ${diagnostics.length}`);
        // Close document
    ...
    // logger.warn(`Timeout waiting for diagnostics for ${uri}, final check: ${finalCheck?.length || 0}`);

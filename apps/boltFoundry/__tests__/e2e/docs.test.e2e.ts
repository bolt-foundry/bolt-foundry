...
Deno.test("renders documentation at /docs route", async () => {
  ...
  try {
    ...
    assert(
      bodyText?.includes("Quickstart Guide") && bodyText?.includes("Getting Started"),
...
Deno.test("renders specific documentation page at /docs/quickstart", async () => {
  ...
  try {
    ...
    logger.info(`Page content length: ${bodyText?.length}, contains 'error': ${bodyText?.toLowerCase().includes('error')}`);
  } finally {
    await teardownE2ETest(context);
  }
});
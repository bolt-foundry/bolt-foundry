...
function parseCards(content: string, depth: number = 0): Card[] {
  ...
  while (currentIndex < content.length) {
    ...
    const arrowMatch = fullCardText.match(/\.card\(\s*"[^"]+"\s*,\s*\([^)]*\)\s*=>\s*/);
    if (!arrowMatch) {
    ...
    // Extract specs
    const specMatches = specsContent.matchAll(/\.spec\(\s*"([^"]+)"(?:\s*,\s*\{([^}]*)\})?/g);
    const specs: Spec[] = [];
    ...
    for (const specMatch of specMatches) {
      ...
      let samples: Sample[] = [];
      if (samplesContent) {
        const sampleMatches = samplesContent.matchAll(/\.sample\(\s*"([^"]+)"\s*,\s*(-?\d+)\)/g);
      ...
      specs.push({ text: specText, samples: samples.length > 0 ? samples : undefined });
    }
...
function parseDeckString(deckString: string): DeckData | null {
  try {
    ...
    const deckLevelContent = firstCardIndex !== -1 ? deckString.slice(0, firstCardIndex) : deckString;
    
    const deckSpecs: Spec[] = [];
    const deckSpecMatches = deckLevelContent.matchAll(/\.spec\(\s*"([^"]+)"(?:\s*,\s*\{([^}]*)\})?/g);
    ...
    for (const specMatch of deckSpecMatches) {
      ...
      deckSpecs.push({ text: specText, samples: samples.length > 0 ? samples : undefined });
    }

    // Extract cards using parser
    const cardsContent = firstCardIndex !== -1 ? deckString.slice(firstCardIndex) : "";
    const cards = parseCards(cardsContent, 0);

    return { name: deckName, specs: deckSpecs, cards };
  } catch (error) {
    console.error('Error parsing deck string:', error);
    return null;
  }
}
            ...
            {spec.samples && (
              <div className="samples">
                ...
                {spec.samples.map((sample, sampleIndex) => (
                  <div key={sampleIndex} className={`sample-item ${sample.score > 0 ? 'positive' : 'negative'}`}>
        ...
        {card.cards && (
          <div className="nested-cards-container">
            {card.cards.map((nestedCard, cardIndex) => (
              <CardRenderer key={cardIndex} card={nestedCard} depth={depth + 1} />
            ))}
          </div>
        )}
      </div>

...
/**
 * 🔑 Login page – built with the same Isograph‐powered pattern used by
 * `EntrypointHome` and other pages. The component is split into:
 *  1. `LoginPage` – the UI + mutation form (annotated with `@component`).
 *  2. `EntrypointLogin` – thin wrapper that exposes `LoginPage` as an
 *     entrypoint for the router + provides the `<title>`.
 *
 *  Place this file alongside `EntrypointHome.ts` so the code‑gen picks it up
 *  automatically (`apps/boltFoundry/entrypoints/EntrypointLogin.ts`).
 */

...
The docs system uses static build-time imports due to ESBuild/Isograph architecture:
1. MDX files in `/docs/` are compiled during build
2. An import map is generated at `apps/boltFoundry/__generated__/docsImportMap.ts`
...
This approach ensures type safety, build-time validation, and optimal performance.
...
### Current Blocker
The runtime rendering fails when trying to display MDX content through Isograph. The error occurs in the `BfIsographFragmentReader` component when attempting to render the documentation.
...
- Consider if docs need the full Isograph treatment or if simpler routing would suffice

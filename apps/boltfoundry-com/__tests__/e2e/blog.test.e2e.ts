...
Deno.test("Blog page loads successfully", async () => {
  ...
  try {
    ...
    const blogStylesLoaded = await context.page.evaluate(() => {
      ...
      const styles = window.getComputedStyle(blogMain);
    ...
    if (videoResult) {
      logger.info("Video recorded:", videoResult.outputPath);
    }
...
Deno.test("Blog post links are properly formatted", async () => {
  ...
  try {
    ...
    for (const link of postLinks) {
      ...
      assert(link.href?.length > 6, "Links should have slug after /blog/");
    }

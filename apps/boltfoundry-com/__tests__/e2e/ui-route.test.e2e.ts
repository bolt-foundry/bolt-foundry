...
Deno.test("UI route renders UIDemo component correctly", async () => {
  ...
  try {
    ...
    assert(
      counterText?.includes("Counter: 0"),
      `Counter should start at 0 in UIDemo, but found: "${counterText}"`,
    );
    ...
    assert(
      incrementButton,
      "Increment button should be visible in UIDemo",
    );
    ...
    assert(
      updatedCounterText?.includes("Counter: 1"),
      "Counter should increment to 1 after button click in UIDemo",
    );

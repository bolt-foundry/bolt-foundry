...
export function JsonDisplay({ data, _schema }: JsonDisplayProps) {
  ...
  const renderArray = (arr: Array<unknown>, title?: string) => {
    ...
    if (
      arr.length > 0 && typeof arr[0] === "object" && !Array.isArray(arr[0])
    ) {
                  ...
                  <tr key={index}>
                    {keys.map((key) => (
                      <td key={key} className="value-cell">
                        {isSimpleValue(item[key])
                          ? (
                            <span className={`value-${typeof item[key]}`}>
  ...
  const renderValue = (value: unknown, title?: string): JSX.Element => {
    ...
    return renderObjectAsTable(value, title);
  };

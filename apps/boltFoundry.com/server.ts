...
import type { getConfigurationVariable as _getConfigurationVariable } from "@bolt-foundry/get-configuration-var";
...
const handler = async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    const fullPath =
      new URL(import.meta.resolve(`@bfmono/build/boltFoundry-com${filePath}`))
        .pathname;
    ...
    try {
      ...
      if (filePath !== "/index.html") {
        const indexPath = new URL(
          import.meta.resolve("@bfmono/build/boltFoundry-com/index.html"),
        ).pathname;

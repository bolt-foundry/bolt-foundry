...
Deno.test("BfDsRange - handles decimal step values correctly", () => {
  const { doc } = render(
    <BfDsRange min={0} max={1} step={0.1} value={0.3} onChange={() => {}} />,
  );
  const input = doc?.querySelector('input[type="range"]');
  assertExists(input);
  assertEquals(input?.getAttribute("step"), "0.1");
  assertEquals(input?.getAttribute("value"), "0.3");
});

Deno.test("BfDsRange - rounds values properly with decimal steps", () => {
  const TestComponent = () => {
  ...
  // The initial value should be properly set
  assertEquals(input?.getAttribute("value"), "0.1");
});

...
async function resumeWorkspace() {
  try {
    ...
    const workspaces = await invoke<Array<{ name: string; running: boolean }>>(
      "list_workspaces",
    );
...
async function refreshFileTree(workspace: string) {
  try {
    const files = await invoke<Array<{ name: string; isDirectory: boolean }>>(
...
globalThis.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("new-workspace-btn")?.addEventListener(
    "click",
    launchNewWorkspace,
  );
  document.getElementById("resume-workspace-btn")?.addEventListener(
    "click",
    resumeWorkspace,
  );
  ...
  document.getElementById("restart-container")?.addEventListener(
    "click",
    restartContainer,
  );
  ...
  terminal?.writeln(
    'Click "New Workspace" to start or "Resume Workspace" to continue.',
  );
});

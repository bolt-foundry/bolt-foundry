...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'x-api-key', 'Cookie', 'Set-Cookie'],
...
app.use(cookieParser(process.env.SESSION_SECRET || 'contact-manager-session-secret'));
...
(async () => {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EADDRINUSE') {
      console.error(`❌ Port ${port} is already in use. Another application might be running on this port.`);
    } else {
  ...
  server.listen({
    ...
    console.log(`🔑 API authentication is ${process.env.API_KEY ? 'ENABLED' : 'MISSING KEY'}`);
    if (process.env.API_KEY) {
      console.log(`API_KEY: "${process.env.API_KEY}" (length: ${process.env.API_KEY.length})`);
    }
    console.log(`🗄️ Database connection is ${process.env.DATABASE_URL_CONTACTS ? 'CONFIGURED' : 'MISSING'}`);
    console.log(`🔐 Session secret is ${process.env.SESSION_SECRET ? 'CONFIGURED' : 'MISSING'}`);
    ...
    if (process.env.SENDGRID_API_KEY) {
      ...
      console.log(`📧 Email: Using test account (emails logged to console only)`);
    }

...
import { CfDsToggle } from "@bfmono/apps/cfDs/components/CfDsToggle.tsx";
import { ListItemExpandContext } from "@bfmono/apps/cfDs/components/CfDsList.tsx";

export type CfDsListItemHandle = {
...
export const CfDsListItem = forwardRef<CfDsListItemHandle, Props>(
  (
    {
      action,
      content,
      ...
      >
        <div className={listItemRowClasses}>
          ...
          {iconRight && !expandedContent && (
            <div className="list-item-icon">
              <CfDsIcon name={iconRight} color="var(--textSecondary)" />
            </div>
          )}
          {toggle && (
            <div className="list-item-toggle">
              <CfDsToggle value={!!toggled} onChange={toggle} />
            </div>
          )}
          {action && (

...
3. **Implement Registry Generator**
   - Create genInterfaces.ts script to scan for interface classes
   - Generate graphqlInterfaces.ts registry file
...
apps/bfDb/
  graphql/
    ...
    __generated__/
      ...
      interfaceImplementers.ts   # Generated mapping of interfaces to implementers
    nodeTypes/
...
function createObjectTypeFromClass(classConstructor) {
  ...
  while (current && current.prototype) {
    ...
    // Check if parent is in our interfaces registry

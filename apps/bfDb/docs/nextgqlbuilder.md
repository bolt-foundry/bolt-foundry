...
`nonNull` works exactly like Nexus: it returns a sub‑builder whose scalar/relation helpers wrap the underlying `GraphQLType` in `GraphQLNonNull`, letting you declare required output fields with fluent syntax.
...
`nonNull` returns a sub‑builder whose scalar helpers wrap the resulting `GraphQLInputType` in `GraphQLNonNull`, giving the familiar Nexus‑style syntax seen in `.nonNull.string(...)`.

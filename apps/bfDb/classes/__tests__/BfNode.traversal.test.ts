...
// Test 4: queryConnectedTargets - finds nodes this node connects to
Deno.test("queryConnectedTargets - finds nodes this node connects to", async () => {
  await withIsolatedDb(async () => {
    const cv = makeLoggedInCv();
    const { org, project } = await setupTestGraphData(cv);

    // Find projects that the organization connects to
    const targets = await org.queryConnectedTargets(TestProject);

...
export abstract class BfNode<TProps extends PropsBase = {}>
  ...
  async createTargetNode<T extends BfNode<Record<string, unknown>>>(
    TargetNodeClass: AnyBfNodeCtor<T>,
    props: InferProps<typeof TargetNodeClass>,
    options?: {
      role?: string;
      metadata?: Partial<BfMetadata>;
    },
  ): Promise<T> {
    const targetNode = await (TargetNodeClass as AnyBfNodeCtor<T>)

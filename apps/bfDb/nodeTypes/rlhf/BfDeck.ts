...
export class BfDeck extends BfNode<InferProps<typeof BfDeck>> {
  /**
   * GraphQL schema specification for BfDeck.
   * Defines the fields exposed via GraphQL API.
   */
  static override gqlSpec = this.defineGqlNode((node) =>
    node
      ...
      .string("systemPrompt")
      .string("description")
    // TODO: Add relationships once BfGrader and BfSample are implemented
    // .connection("graders", () => import("../BfGrader.ts").then(m => m.BfGrader))
    // .connection("samples", () => import("../BfSample.ts").then(m => m.BfSample))
  );

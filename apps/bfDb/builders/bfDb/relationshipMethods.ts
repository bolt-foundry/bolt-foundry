...
export function generateRelationshipMethods<T extends AnyBfNodeCtor>(
  ...
  const spec = (nodeClass as any).bfNodeSpec;
  ...
  for (const [relationName, relationSpec] of Object.entries(spec.relations)) {
    ...
    (instance as any)[`find${methodBaseName}`] = async function () {
    ...
    (instance as any)[`create${methodBaseName}`] = async function (props: any) {
      const TargetClass = await relation.target();
    ...
    (instance as any)[`delete${methodBaseName}`] = async function (
      options?: { deleteNode?: boolean },
    ) {

        ...
        if (relation._targetThunk) {
          ...
          if (!relation.type || relation.type === relationName) {
            ...
            if (
              ...
              relation.type = classMatch[1];
              logger.debug(
                `Extracted type name ${relation.type} from thunk for relation ${relationName}`,
              );
            } else {
              // If we can't determine the type but it's a common pattern, apply some heuristics
              if (relationName === "currentViewer") {
                relation.type = "CurrentViewer";
                logger.debug(
                  `Using conventional type name CurrentViewer for relation currentViewer`,
                );
              } else {
                // Capitalize the first letter as a convention
                relation.type = relationName.charAt(0).toUpperCase() +
                  relationName.slice(1);
                logger.debug(
                  `Using capitalized relation name ${relation.type} as type for relation ${relationName}`,
                );
              }
            }
          }

...
// Define MaybePromise locally since it's not exported from lib/types.ts
  ...
  return async function edgeRelationshipResolver(
    ...
    try {
      ...
      if (isSourceToTarget) {
        ...
        // Use a type assertion to handle the complex type constraints
        // This is safe because we know the structure of the query parameters
        const edges = await (BfEdgeModule.BfEdge as any).query(
          root.currentViewer,

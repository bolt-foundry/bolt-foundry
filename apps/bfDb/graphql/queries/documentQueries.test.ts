...
Deno.test("documentsBySlug resolver handles explicit slug", async () => {
  ...
  const _result = await resolver({}, { slug: "api-reference" });
...
Deno.test("documents resolver handles category filtering", async () => {
  ...
  const _allDocsResult = await resolver({}, {});

  // Test with category filter
  const _categoryResult = await resolver({}, { category: "api" });
...
Deno.test("documents resolver handles published filter", async () => {
  ...
  const _publishedResult = await resolver({}, { published: true });
  const _draftResult = await resolver({}, { published: false });
...
Deno.test("documents resolver handles pagination", async () => {
  ...
  const _firstPageResult = await resolver({}, { first: 2 });
  const _nextPageResult = await resolver({}, { first: 2, after: "cursor1" });

  // Test backward pagination
  const _lastPageResult = await resolver({}, { last: 2 });
  const _prevPageResult = await resolver({}, { last: 2, before: "cursor2" });
...
Deno.test("documentContent resolver returns content string", async () => {
  ...
  const _content = await resolver({}, { slug: "getting-started" });
...
Deno.test("documentContent resolver handles missing document", async () => {
  ...
  const _content = await resolver({}, { slug: "non-existent-doc" });
...
Deno.test("document resolvers handle errors gracefully", async () => {
  ...
  const _doc = await documentsBySlugResolver({}, { slug: "invalid" });
  const _docs = await documentsResolver({}, { category: "nonexistent" });
  const _content = await documentContentResolver({}, { slug: "invalid" });
...
Deno.test("fragment supports different document types", async () => {
  ...
  const _guidesDocs = await resolver({}, { category: "guides" });
  const _apiDocs = await resolver({}, { category: "api" });
  const _tutorialDocs = await resolver({}, { category: "tutorials" });
...
Deno.test("fragment supports advanced query features", async () => {
  ...
  const _complexResult = await resolver({}, {
    category: "guides",
    published: true,
    first: 5,
    after: "cursor123",
...
Deno.test("document content resolver supports markdown processing", async () => {
  ...
  const _markdownContent = await resolver({}, { slug: "markdown-example" });
...
Deno.test("fragment handles document metadata", async () => {
  ...
  const resolver =
    (documentQueries.fields.documentsBySlug as DocumentQueryResolver).resolve;
  const _doc = await resolver({}, { slug: "test-doc" });

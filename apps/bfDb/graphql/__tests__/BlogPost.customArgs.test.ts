...
Deno.test("GraphQL blogPosts connection should support sortDirection argument", async () => {
  ...
  try {
    ...
    const blogPostsAsc = ascResult.data.blogPosts as {
    ...
    const blogPostsDesc = descResult.data!.blogPosts as {
      edges: Array<{ node: { id: string } }>;
    };
...
Deno.test("GraphQL blogPosts connection should support filterByYear argument", async () => {
  ...
  try {
    ...
    const blogPostsData = result.data.blogPosts as {
      edges: Array<{ node: { id: string; content: string } }>;
    };
    const edges = blogPostsData.edges;

    // There are only 2 test posts from 2024 since all old posts were deleted
    assertEquals(
      edges.length,
      2,
      "Should have 2 posts from 2024 (test posts only)",
    );
...
Deno.test("GraphQL schema introspection should show custom args", async () => {
  ...
  const typeInfo = result.data.__type as {
    fields: Array<
      {
        name: string;
        args: Array<
          { name: string; type: { name?: string; ofType?: { name?: string } } }
        >;
      }
    >;
  };

...
Deno.test({
  ...
  if (result.data?.joinWaitlist) {
    assert(
      typeof result.data.joinWaitlist.success === "boolean",
      "success should be a boolean",
    );
    assert(
      result.data.joinWaitlist.message === null ||
        typeof result.data.joinWaitlist.message === "string",

...
Deno.test("fragment system preserves existing schema structure", async () => {
  // Test that fragment-based schema can be drop-in replacement
  const currentSchema = await buildTestSchema();
  const currentSDL = printSchema(currentSchema);

  // Current schema should have expected structure
  assert(currentSDL.includes("type Query"));
  assert(currentSDL.includes("blogPost"));
  assert(currentSDL.includes("documentsBySlug"));

  // Fragment composition should produce equivalent structure
  const allFragments = composeFragmentFields([
    "blogPostQueries",
    "documentQueries",
    "githubQueries",
    "coreQueries",
  ...
  // Same field count
  assertEquals(
    Object.keys(allFragments).length,
    Object.keys(currentQueryFields).length,
  );
});

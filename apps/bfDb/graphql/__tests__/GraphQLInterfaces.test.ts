...
Deno.test("GraphQL interface registry file exists", async () => {
  try {
    ...
    const path = "/home/runner/workspace/apps/bfDb/graphql/__generated__/graphqlInterfaces.ts";
    ...
    throw new Error(
      `Interface registry file does not exist or has invalid content: ${error instanceof Error ? error.message : String(error)}`
    );
  }
});
...
Deno.test("Interface registry is loaded by schema builder", async () => {
  ...
  const nodeInterface = types.find((type: any) => 
    (type.name === "Node" || type.name === "GraphQLNode") && 
    type.kind === "interface"
  );
  ...
  // deno-lint-ignore no-explicit-any
  assert((nodeInterface as any).definition, "Node interface should have a definition");
});

...
export function loadGqlTypes() {
  ...
  // We need to merge them all into a single extendType to prevent duplicates
  if (mutationExtensions.length > 0) {
    logger.debug(`Adding ${mutationExtensions.length} mutation extensions`);
    
    // Create a single merged mutation extension
    const mergedMutationType = {
      type: "Mutation",
      definition(t: any) {
        // Apply each mutation extension's definition
        mutationExtensions.forEach((mutationExt) => {
          if (typeof mutationExt.definition === "function") {
            mutationExt.definition(t);
          }
        });
      },
    };
    
    // Add the merged extension only once
    types.push(extendType(mergedMutationType));
  }

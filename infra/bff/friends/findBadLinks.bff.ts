...
export default async function findBadLinksCommand(
  ...
  info(chalk.blue("🔍 Searching for broken markdown links...\n"));
  ...
  if (brokenLinks.length === 0) {
    ...
    for (const [file, links] of linksByFile) {
      ...
      for (const link of links) {
        info(`  Line ${link.line}: ${chalk.red(link.link)}`);
        if (link.suggestedFix) {
          info(`    → Suggested: ${chalk.green(link.suggestedFix)}`);
        }
      }
    }
  }
  ...
  if (flags.fix && brokenLinks.length > 0) {
    ...
    if (fixable.length > 0) {
      ...
      info(chalk.green(`✅ Fixed ${fixable.length} links`));
    }
  }
...
async function aiFixBrokenLinks(brokenLinks: Array<BrokenLink>) {
  ...
  logger.info(chalk.yellow("ℹ️  Using automatic suggestions to fix links..."));
  ...
  if (fixedLinks.length > 0) {
    ...
    logger.info(chalk.green(`✅ Fixed ${fixedLinks.length} links`));
  }

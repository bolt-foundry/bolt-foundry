...
export async function mergeStack(args: string[]): Promise<number> {
  ...
  async function getRepoInfo(): Promise<{ owner: string; repo: string } | null> {
    const { stdout: repoUrlOutput, code: repoUrlCode } = await runShellCommandWithOutput([
      "sl",
      "paths",
      "--template",
      "{url}",
  ...
  async function getCurrentCommitStack(): Promise<string[]> {
    ...
    for (const line of lines) {
      // Skip lines that don't contain commit hashes
      if (!line.match(/^[a-f0-9]{12}/)) continue;
      
      const [hash, ...descParts] = line.split(" ");
  ...
  async function getCommitPRNumber(commitHash: string): Promise<string | null> {
    // Get the commit message to look for PR info
    const { stdout, code } = await runShellCommandWithOutput([
      "sl",
      "log",
      "-r",
    ...
    // Look for "Differential Revision:" or similar patterns that indicate a PR
    const desc = stdout.trim();
    
    // Try to find PR number in commit message or use sl pr list
    const { stdout: prListOutput } = await runShellCommandWithOutput(["sl", "pr", "list"]);
    const prLines = prListOutput.split("\n").filter(line => line.trim());
    
    for (const line of prLines) {
      if (line.includes("OPEN") && line.includes(commitHash.substring(0, 8))) {
  ...
  async function isPRReadyToMerge(prNumber: string): Promise<{ ready: boolean; reason?: string }> {
    ...
    if (pr.mergeable_state === "blocked") {
      return { ready: false, reason: "PR is blocked (likely by required checks)" };
    }
    ...
    const { stdout: checksOutput, code: checksCode } = await runShellCommandWithOutput([
      "gh",
      "api",
    ...
    if (checksCode === 0) {
      try {
        ...
        const failedChecks = checkRuns.filter((check: any) => 
        ...
        if (failedChecks.length > 0) {
          return { 
            ready: false, 
            reason: `Failed checks: ${failedChecks.map((c: any) => c.name).join(", ")}` 
          };
        }

        // Check if any checks are still pending
        const pendingChecks = checkRuns.filter((check: any) => 
        ...
        if (pendingChecks.length > 0) {
          return { 
            ready: false, 
            reason: `Pending checks: ${pendingChecks.map((c: any) => c.name).join(", ")}` 
          };
        }
      } catch (e) {
  ...
  async function mergePR(prNumber: string, method: string): Promise<boolean> {
    ...
    const { stdout: mergeOutput, code: mergeCode } = await runShellCommandWithOutput([
      "gh",
      "api",
  ...
  async function getBottomCommitOfStack(stackCommits: string[]): Promise<string | null> {
  ...
  async function navigateToBottomOfStack(): Promise<boolean> {
    ...
    const bottomCommit = await getBottomCommitOfStack(commits);
    ...
    if (currentHash.startsWith(bottomCommit) || bottomCommit.startsWith(currentHash)) {
      logger.info("Already at bottom of stack");
      return true;
    }

    // Navigate to bottom of stack
    logger.info(`Navigating to bottom of stack (commit ${bottomCommit.substring(0, 12)}...)`);
    ...
    const { code: gotoCode } = await runShellCommandWithOutput([
      "sl", "goto", bottomCommit.substring(0, 12)
    ]);
  ...
  if (stackPRs.length === 0) {
    logger.info("No PRs found in current stack. Make sure you're on a commit with an associated PR.");
    return 0;
  }

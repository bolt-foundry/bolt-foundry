...
export async function mergeStack(args: string[]): Promise<number> {
  ...
  async function getCommitPRNumber(commitHash: string): Promise<string | null> {
    ...
    const { stdout: descOutput, code: descCode } =
      await runShellCommandWithOutput([
        "sl",
        "log",
        "-r",
    ...
    const { stdout: branchOutput, code: branchCode } =
      await runShellCommandWithOutput([
        "sl",
        "log",
        "-r",
  ...
  async function isPRReadyToMerge(
    ...
    logger.warn(
      `Unknown mergeable_state: ${pr.mergeable_state}, falling back to check-runs`,
    );
  ...
  async function waitForPRChecks(prNumber: string): Promise<boolean> {
    ...
    while (true) {
      ...
      if (readiness.ready) {
        logger.info(
          `✅ PR #${prNumber} checks have completed and PR is ready!`,
        );
        return true;
      }
      ...
      if (readiness.reason && !readiness.reason.includes("Pending checks")) {
        ...
        logger.warn(
          `This is not a pending check issue, so we won't wait for it.`,
        );
        return false;
      }
      ...
      logger.info(
        `⏳ PR #${prNumber}: ${
          readiness.reason || "Still pending"
  ...
  async function waitForAllPRChecks(prNumbers: string[]): Promise<string[]> {
    ...
    for (const prNumber of prNumbers) {
      ...
      if (isReady) {
        ...
        logger.info(
          `Stopping wait at PR #${prNumber} - not a pending check issue.`,
        );
        break;
      }
    }
  ...
  if (waitForChecks && !dryRun) {
    ...
    const pendingChecksPRs = notReadyPRs.filter((p) =>
      p.reason &&
      (p.reason.includes("Pending checks") ||
        p.reason.includes("PR is blocked"))
    );
    ...
    if (pendingChecksPRs.length > 0) {
      ...
      const _readyAfterWait = await waitForAllPRChecks(prsToWaitFor);

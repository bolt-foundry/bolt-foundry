      ...
      if (resultsWithGroundTruth.length > 0) {
        ...
        if (disagreements.length > 0) {
          ...
          printTable(
            disagreements.map(result => {
              ...
              return {
                ...
                "Assistant Response": result.sample?.assistantResponse || "N/A"
              };
            })
          );
          ...
          disagreements.forEach((result, index) => {
            ...
            printLine(`${"=".repeat(80)}`);
            printLine(`User Input: ${result.sample?.userMessage || "N/A"}`);
            printLine(`Assistant Output: ${result.sample?.assistantResponse || "N/A"}`);
            printLine(`Scores: Judge=${result.score}, Ground Truth=${groundTruth}`);
            printLine(`Judge Notes: ${result.output.notes || "No notes provided"}`);
          });
        }
      }
    }

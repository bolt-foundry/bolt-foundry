...
export async function aiAmend(args: string[]): Promise<number> {
  logger.info("Running aiAmend to amend previous commit with AI-generated message...");
  ...
  try {
    ...
    const { stdout: userInfoJson, code: userInfoCode } = await runShellCommandWithOutput(
      ["gh", "api", "user"],
      {},
      true,
      true
  ...
  const { stdout: currentCommitMessage, code: commitMsgCode } = await runShellCommandWithOutput(
  ...
  const { stdout: commitDiff, code: commitDiffCode } = await runShellCommandWithOutput(
    ["sl", "diff", "-c", "."],
    {},
    true,
    true
  ...
  const { stdout: uncommittedDiff, code: uncommittedDiffCode } = await runShellCommandWithOutput(
    ["sl", "diff"],
    {},
    true,
    true
  ...
  const combinedDiff = commitDiff + (uncommittedDiff.trim() ? "\n\n/* Uncommitted Changes */\n" + uncommittedDiff : "");

...
export async function isoCommand(options: Array<string>): Promise<number> {
  ...
  try {
    ...
    for (const workingDir of appsWithIsograph) {
      ...
      if (verbose) {
        // Change to working directory and run command with output capture
        ...
        if (code === 0) {
          logger.info(
            `✅ Isograph compilation for ${workingDir} completed successfully`,
          );
        } else {
          logger.error(
            `❌ Isograph compilation for ${workingDir} failed with code ${code}`,
          );
          overallResult = code;
        }
      } else {
        ...
        if (result === 0) {
          ...
          logger.error(
            `❌ Isograph compilation for ${workingDir} failed with code ${result}`,
          );

...
function parseDenoLintJsonOutput(jsonString: string) {
  try {
    ...
    logger.debug("Raw lint JSON output sample (first 500 chars):", 
    ...
    if (parsed.diagnostics?.length > 0) {
      logger.debug("First diagnostic object structure:", JSON.stringify(parsed.diagnostics[0], null, 2));
    } else {
      logger.debug("No diagnostics found in lint output");
    }
    
    if (parsed.errors?.length > 0) {
      logger.debug("First error object structure:", JSON.stringify(parsed.errors[0], null, 2));
    } else {
    ...
    for (const diag of parsed.diagnostics ?? []) {
      ...
      logger.debug(`Processing diagnostic: ${filePath}:${line}:${col} - ${message.substring(0, 50)}...`);
    ...
    for (const err of parsed.errors ?? []) {
      ...
      logger.debug(`Processing error: ${filePath}:${line}:${col} - ${message.substring(0, 50)}...`);
...
function parseDenoFmtOutput(fullOutput: string) {
  ...
  logger.debug("Format check output sample (first 500 chars):", 
  ...
  for (const line of lines) {
    ...
    if (currentFile) {
      ...
      if (lineMatch) {
        ...
        logger.debug(`Found format issue at ${currentFile}:${lineNum} - ${snippet.substring(0, 50)}`);

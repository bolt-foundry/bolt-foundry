      ...
      if (entry.isFile) {
        ...
        if (entry.name.endsWith(".bft.ts")) {
          ...
          try {
            ...
            if (taskDef && typeof taskDef.fn === "function" && taskDef.description) {
              taskMap.set(taskName, taskDef);
            } else {
              ui.error(`Task ${entry.name} does not export a valid bftDefinition`);
            }
          } catch (error) {
...
function createDeckRunner(deckPath: string): (args: Array<string>) => Promise<number> {

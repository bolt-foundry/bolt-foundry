import { runShellCommand, runShellCommandWithOutput } from "@bfmono/infra/bff/shellBase.ts";
...
async function getChangedFiles(): Promise<Array<string>> {
  const { stdout: statusOutput, code: statusCode } = await runShellCommandWithOutput(["sl", "status"]);
 ...
 */
async function getValidationTasks(files?: Array<string>): Promise<Array<{ name: string; command: Array<string> }>> {
  const tasks: Array<{ name: string; command: Array<string> }> = [];
  
  // If specific files provided, use those. Otherwise get all changed files
  const filesToCheck = files && files.length > 0 ? files : await getChangedFiles();
  ...
  const hasConfigFiles = filesToCheck.some(file => 
    file.includes('package.json') || file.includes('deno.json') || file.includes('deno.lock')
  );
 ...
 */
async function runSmartValidation(files?: Array<string>, skipPrecommit = false): Promise<boolean> {
  if (skipPrecommit) {
    return true;
  }
  ...
  for (const task of tasks) {
    ...
    const result = await runShellCommand(task.command, undefined, {}, true, true); // silent mode
...
async function stageFiles(files?: Array<string>): Promise<number> {
  if (files && files.length > 0) {
    ...
    if (deletedFiles.length > 0) {
      const removeResult = await runShellCommand(["sl", "remove", "--mark", ...deletedFiles]);
      if (removeResult !== 0) {
        return removeResult;
      }
    }
...
async function handleCommit(args: Array<string>): Promise<number> {
  ...
  if (!commitMessage) {
    logger.error('❌ No commit message provided. Usage: bft sl commit -m "Your message" [--skip-precommit] [--no-submit] [files...]');
    return 1;
  }
  ...
  // Stage files
  const stageResult = await stageFiles(filesToCommit.length > 0 ? filesToCommit : undefined);
  if (stageResult !== 0) {
...
export const bftDefinition = {
  ...
  fn: slCommand,
  aiSafe: false, // Not AI-safe because it can create commits

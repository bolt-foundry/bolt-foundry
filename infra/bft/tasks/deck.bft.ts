...
async function deck(args: Array<string>): Promise<number> {
  if (args.length === 0) {
    ...
    ui.info("Commands:");
    ui.info("  run <deck.md> [--context value]  - Run a deck with optional context");
    ui.info("  render <deck.md>                 - Render a deck file to OpenAI format");
...
async function runDeck(args: Array<string>): Promise<number> {
  if (args.length === 0) {
    ...
    ui.info("Example: bft deck run grader.deck.md --userMessage 'hello' --assistantResponse 'hello world'");
    return 1;
  }
  ...
  try {
    ...
    const request = await buildDeckRequest(deckContent, deckPath, contexts, contextValues);
      ...
      // Handle TOML files with context definitions
        ...
        if (tomlData.contexts && typeof tomlData.contexts === "object") {
          ...
          for (const [key, value] of Object.entries(contextsSection)) {
            if (typeof value === "object" && value !== null && "assistantQuestion" in value) {
...
async function buildDeckRequest(
  ...
  let systemContent = processedMarkdown
    .replace(/^\+\+\+[\s\S]*?\+\+\+\n/, "") // Remove frontmatter

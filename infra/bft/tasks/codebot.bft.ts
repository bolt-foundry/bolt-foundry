...
By default, starts Claude Code CLI. Use options for alternative modes:
...
EXAMPLES:
  bft codebot                 # Start Claude Code CLI (default)
  bft codebot --shell         # Open container shell
  ...
  const copyWorkspacePromise = (async () => {
    ...
    try {
      ...
      const copyClaudeJson = new Deno.Command("cp", {
        args: ["--reflink=auto", `${homeDir}/.claude.json`, `${workspacePath}/claude.json`],
      });
    ...
    try {
      ...
      const copyClaudeDir = new Deno.Command("cp", {
        args: ["--reflink=auto", "-R", `${homeDir}/.claude`, `${workspacePath}/claude`],
      });
    ...
    for (const result of copyResults) {
      if (!result.success) {
        ...
        logger.info("🔍 WORKSPACE COPY FAILURE STACK TRACE:", new Error().stack || "No stack trace available");
        ui.output("🔍 TRACE: Workspace copy failed, indicating duplicate execution");
      ...
      if (!inspectResult.success || inspectOutput === "[]") {
        ...
        try {
          ...
          if (imageCreatedStr) {
            ...
            const dockerfileStat = await Deno.stat("infra/apps/codebot/Dockerfile");

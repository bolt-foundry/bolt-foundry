...
async function syncSecretsFromOnePassword(
  ...
  try {
    ...
    logger.error(`Failed to sync secrets: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
}
...
async function selectVault(): Promise<string | null> {
  try {
    ...
    logger.error(`Failed to select vault: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
...
async function generateTypes(): Promise<number> {
  try {
    ...
    ui.warn(`⚠️  Failed to generate TypeScript definitions: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
}

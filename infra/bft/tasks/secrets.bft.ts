...
import { parseEnvFile, formatEnvFile } from "@bfmono/packages/env/utils.ts";
...
async function secretsTask(args: Array<string>): Promise<number> {
  if (args.length === 0) {
    ...
    ui.info("Commands:");
    ui.info("  sync                 - Sync all variables to .env.client and .env.server");
    ui.info("  sync --client-only   - Sync only client variables to .env.client");
    ui.info("  sync --server-only   - Sync only server variables to .env.server");
    ui.info("  list                 - List all available environment variables");
    ui.info("  generate-types       - Generate TypeScript definitions from example files");
    return 1;
  }
...
async function syncSecretsFromOnePassword(args: Array<string>): Promise<number> {
  ...
  if (clientKeys.length === 0 && serverKeys.length === 0) {
    ui.error("No .env.example files found. Create .env.client.example and .env.server.example first.");
    return 1;
  }
  ...
  try {
    ...
    if (opCheckCode !== 0) {
      logger.warn("1Password CLI not available. Creating empty env files for development.");
      await writeEnvFiles(keysToFetch, {}, clientKeys, serverKeys, clientOnly, serverOnly);
      ui.info(`Created empty env files for ${keysToFetch.length} variables`);
      ui.info("Install 1Password CLI and run 'bft secrets sync --force' to populate with real values");
...
async function listAvailableSecrets(): Promise<number> {
  ...
  if (clientKeys.length === 0 && serverKeys.length === 0) {
    ...
    ui.info("Create .env.client.example and .env.server.example to define your environment variables.");
    return 1;
  }
  ...
  ui.info(`Total: ${totalKeys} variables (${clientKeys.length} client, ${serverKeys.length} server)`);
  return 0;
}
...
async function selectVault(): Promise<string | null> {
  try {
    ...
    if (vaults.length === 0) {
      ui.error("No 1Password vaults found. Make sure you're signed in with 'op signin'");
      return null;
    }
    ...
    if (bfVault) {
      ui.info(`Auto-selected Bolt Foundry vault: ${bfVault.name} (${bfVault.id})`);
      return bfVault.id;
    }
    ...
    if (selection >= 1 && selection <= vaults.length) {
      ...
      ui.info(`To avoid this prompt, set: export BF_VAULT_ID=${selectedVault.id}`);
...
async function generateTypes(): Promise<number> {
  try {
    const { generateEnvTypes } = await import("@bfmono/packages/env/generate-types.ts");
    ...
    ui.warn(`⚠️  Failed to generate TypeScript definitions: ${error.message}`);
    return 1;
  }
}

...
async function claudify(_args: Array<string>): Promise<number> {
  ...
  const claudeCommandsDir = join(import.meta.dirname!, "../../../.claude/commands/bft");
      ...
      if (entry.isFile) {
        if (entry.name.endsWith(".bft.ts")) {
          ...
          try {
            ...
            if (frontmatterMatch) {
              try {
                const frontmatter = parseTOML(frontmatterMatch[1]) as Record<string, unknown>;
                if (frontmatter.meta && typeof frontmatter.meta === "object" && "purpose" in frontmatter.meta) {
            ...
            const command = await generateCommandForDeck(taskName, purpose, deckPath);
            commands.push(command);
...
function generateCommandForTask(taskName: string, taskDef: TaskDefinition): ClaudeCommand {
...
async function generateCommandForDeck(taskName: string, purpose: string, deckPath: string): Promise<ClaudeCommand> {
  const name = `bft-${taskName}`;
...
async function getContextParamsForDeck(taskName: string, deckPath: string): Promise<Array<ContextParam>> {
      ...
      if (tomlData.contexts && typeof tomlData.contexts === "object") {
        ...
        for (const [key, value] of Object.entries(contexts)) {
          if (typeof value === "object" && value !== null) {
            ...
            params.push({
              name: key,
              description: String(ctx.description || ctx.assistantQuestion || key),

...
export function render(node: React.ReactNode) {
  ...
  return {
    ...
    findConfirmationButtons: (containerId: string) => {
      ...
      if (container && container.getAttribute('data-confirmation-visible') === 'true') {
        return {
          confirmButton: container.querySelector('[data-testid="confirm-button"]'),
          cancelButton: container.querySelector('[data-testid="cancel-button"]'),
        };
      }
      return {
        confirmButton: null,

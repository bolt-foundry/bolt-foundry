...
interface StorageOptions {
  strategy: "memory" | "server" | "custom";
  customStorage?: CustomStorageImplementation;
  batchSize?: number;
}

// Provider-specific client wrapper
interface BoltWrappedClient {
...
interface RoutingRule {
  ...
  targetModel: string | ((request: any) => string);
  priority: number;
}

// LLM interaction data schema

...
A core principle in Content Foundry's URL and routing structure is: **"IDs are for computers, slugs are for humans"**. This means:

- Internal operations like database lookups use system-generated IDs (`BfGid` types)
- User-facing URLs use human-readable slugs (e.g., `/blog/my-post-title`)
- Dynamic routes should prefer slug parameters over ID parameters when intended for end users
- The system handles mapping between slugs and IDs internally

This approach creates more memorable and SEO-friendly URLs while maintaining the performance and reliability of ID-based lookups in the backend. When designing new routes or features, consider whether the URL component is primarily for human or computer consumption and choose the appropriate format.

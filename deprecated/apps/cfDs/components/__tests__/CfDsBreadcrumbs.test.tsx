...
import { CfDsBreadcrumbs } from "../CfDsBreadcrumbs.tsx";

Deno.test("CfDsBreadcrumbs renders with crumbs", () => {
  const crumbs = [
    { name: "Page One", link: "/page1" },
    { name: "Page Two", link: "/page2" },
  ];
  const { doc } = render(<CfDsBreadcrumbs crumbs={crumbs} />);
...
Deno.test("CfDsBreadcrumbs renders with homeLink", () => {
  const crumbs = [{ name: "Page One", link: "/page1" }];
  const homeLink = "/home";
  const { doc } = render(
    <CfDsBreadcrumbs crumbs={crumbs} homeLink={homeLink} />,
  );
...
Deno.test("CfDsBreadcrumbs renders back icon for crumb with back=true", () => {
  const crumbs = [
...
Deno.test("CfDsBreadcrumbs renders with correct accessibility attributes", () => {
  const crumbs = [{ name: "Page One", link: "/page1" }];
  const { doc } = render(<CfDsBreadcrumbs crumbs={crumbs} />);

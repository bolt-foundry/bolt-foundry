...
async function generateHintWithClaude(
  ...
  const assistantCard = getHintAssistantCard(
    question,
    userCode,
    previousHints,
    userProgress,
...
function getHintAssistantCard(
  ...
  const assistant = bf.createAssistantCard(
    "javascript-tutor-hints",
    (card) =>
      card
        .spec(
          "You are a patient JavaScript tutor helping students with coding problems",
        )
        .spec(
          "Provide helpful hints without giving away the complete solution too early",
        )
        .spec(
          "Use the Socratic method for students making progress, be more direct for struggling students",
        )
        .spec(
          "Always be encouraging and explain the reasoning behind suggestions",
        )
        .specs("Behaviors", (behavior) => (
          behavior
            ...
            .spec(
              "Use appropriate hint types: 'socratic' for guided discovery, 'direct' for struggling students, 'example' for code examples",
            )
            .spec(
              "Provide encouragement that acknowledges their effort and progress",
            )
            .spec(
              "If student is very stuck, consider showing the solution with explanation",
            )
        ))
        .context((ctx) =>
          ctx
            .string(
              "questionTitle",
              "What is the title of the coding question?",
            )
            .string(
              "questionDescription",
              "What is the description of the coding question?",
            )
            .string(
              "questionConcepts",
              "What concepts does this question cover?",
            )
            .string(
              "questionDifficulty",
            ...
            .boolean(
              "isStruggling",
              "Is the student struggling with this problem?",
            )
        )
        .context((ctx) =>
          ctx
            .number(
              "questionsCompleted",
              "How many questions has the student completed?",
            )
            .number(
              "currentStreak",
            ...
            .string(
              "conceptsLearned",
              "Which concepts has the student already learned?",
            )
            .string("attemptHistory", "What attempts has the student made?")
            .string(
              "jsonFormat",
              "What is the exact JSON format for the hint response?",
            )
        ),
  );
  ...
  const contextData = {
    ...
    previousHints: previousHints.length > 0
      ? previousHints.map((hint, i) => `${i + 1}. ${hint.text}`).join("\\n")
      : "None yet",
    hintsUsed: hintsUsed,
    ...
    attemptHistory: attemptHistory.length > 0
      ? attemptHistory.map((attempt, i) =>
        `Attempt ${i + 1}: ${attempt.wasSuccessful ? "Success" : "Failed"} - ${
          attempt.code.substring(0, 100)
        }...`
      ).join("\\n")
      : "First attempt",
    jsonFormat: `{
...
function parseHintResponse(responseText: string): HintResponse {
  try {
    ...
    return {
      hints: [{
        text:
          "Let me help you think through this step by step. What do you think the function should do first?",
        level: 1,
        type: "socratic",
      }],

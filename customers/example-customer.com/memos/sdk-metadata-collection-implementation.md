...
async function readLocalDeck(path: string, options?: {
  ...
  const apiEndpoint = options?.apiEndpoint || "https://boltfoundry.com/api";
...
interface RenderOptions {
  ...
  // Model parameters can be overridden here
  temperature?: number;
  model?: string;
...
- [ ] Test metadata inclusion - render with captureTelemetry:true
...
- [ ] Test contextVariables capture - Verify params become contextVariables

##### Implementation

- [ ] Add RenderOptions interface with captureTelemetry field
...
import { BfClient, readLocalDeck } from "../bolt-foundry.ts";
...
Extract key fields from invoices.`);
    ...
    const completion = deck.render({ invoice: "INV-001" }, {
      attributes: { invoiceImage: "base64..." },
    });

    assertEquals(completion.bfMetadata?.deckName, "invoice-extraction");
...
const completion: ChatCompletionCreateParams = deck.render({ input: "data" });
...
const completion: ChatCompletionCreateParams = deck.render({
  invoice: invoiceText,
}, {
  attributes: { invoiceImage: base64Data }, // Supplemental data still included
  temperature: 0.1,
});
...
- **Limitation**: Telemetry and metadata collection only work with non-streaming
  requests
- **Workaround**: Collect the final streaming response and manually send to
  telemetry API
...
- **Deck Management**: `/api/decks` endpoint for checking/creating decks
